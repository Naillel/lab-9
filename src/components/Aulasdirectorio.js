...
class AulaItem extends HTMLElement {
    ...
    #label = "";
    #icono = "→";
    #index = 0;  // para el delay de animación escalonada
    ...
    connectedCallback() {
        ...
        // Calcular posición dentro del padre para animar en cascada
...
class AulasDirectorio extends HTMLElement {
    ...
    /* Guardamos los datos de los <aula-item> hijos antes de re-renderizar */
    #items = [];

    constructor() {
        super();
        // ✅ El constructor tiene acceso al HTML declarado en el documento
        //    ANTES de que connectedCallback borre el contenido con render().
        //    Usamos un <template> temporal para parsear los hijos de forma segura.
    ...
    /* Genera las filas <aula-item> desde los datos guardados */
    get #itemsHTML() {
        return this.#items
            .map(({ label, icono }) =>
        ...
        @scope {
            ...
            .ad-card::before {
                ...
                background: linear-gradient(
                    90deg,
                    #4f7bdc 0%, #a5c0f7 40%, #4f7bdc 80%, #2b4fa8 100%
                );
                z-index: 5;
            }
    ...
    render() {
        this.setHTMLUnsafe(this.#template);
    }
}
