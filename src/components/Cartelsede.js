class CartelSede extends HTMLElement {
    ...
    /* ── Mapa atributo → propiedad privada ── */
    #attrMap = {
    ...
    connectedCallback() {
        // Leer todos los atributos HTML y guardar en propiedades privadas
    ...
    /* ── Ciclo de vida: atributo cambiado ── */
    attributeChangedCallback(name, oldValue, newValue) {
        if (oldValue === newValue) return;
        const setter = this.#attrMap[name];
        if (setter) setter(newValue);
        // Solo re-renderizar si ya está en el DOM
        if (this.isConnected) this.render();
    }

    /* ── CSS del componente encapsulado con @scope ── */
    get #styles() {
        return /* css */`
        <style>
        ...
        @scope {
            ...
            /* Animación de entrada del cartel completo */
            @keyframes card-in {
